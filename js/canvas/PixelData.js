...
const PixelData = (function() {
    ...
    const compression = window.Compression || null;
    ...
    function resize(newWidth, newHeight) {
        ...
        if (window.ValidationUtils) {
            const validation = window.ValidationUtils.validateCanvasDimensions(newWidth, newHeight);
            if (!validation.valid) {
    ...
    function exportToString(compress = false) {
        if (!colorPalette) {
            logger.error?.('ColorPalette not available');
            return '';
        }
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                dataString += window.ColorPalette.getBase64Char(data[y][x]);
            }
        }
    ...
    function importFromString(str) {
        try {
            // Validate format
            if (window.ValidationUtils) {
                const validation = window.ValidationUtils.validateDataString(str);
                if (!validation.valid) {
            ...
            if (!colorPalette) {
                return { success: false, error: 'ColorPalette not available' };
            }
            ...
            for (let y = 0; y < height; y++) {
                for (let x = 0; x < width; x++) {
                    ...
                    const colorIndex = window.ColorPalette.getIndexFromChar(char);
